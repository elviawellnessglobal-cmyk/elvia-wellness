...
/* ── PRIVATE / APP-ONLY PAGES ── code-split so the public storefront bundle stays small
   and the server-side prerender never has to import admin-only libraries (tiptap etc.) */
...
const EmailAnalytics = lazy(() => import("./admin/EmailAnalytics"));
const Chats = lazy(() => import("./admin/Chats"));
const ChatDetail = lazy(() => import("./admin/ChatDetail"));
const AdminBlogs = lazy(() => import("./admin/AdminBlogs"));

/* Paths that must never appear in Google (carts, accounts, admin, order pages…) */
const PRIVATE_PATHS = [
  "/admin", "/cart", "/checkout", "/account", "/addresses", "/my-orders",
  "/previous-orders", "/order", "/order-success", "/support", "/success", "/verify",
];
const isPrivatePath = (p) => PRIVATE_PATHS.some((x) => p === x || p.startsWith(x + "/"));
